...
> Disk GetDisk(ctx, id).NoEmbed(noEmbed).Prefer(prefer).Execute()
...
func main() {
	...
	resp, r, err := apiClient.DisksAPI.GetDisk(context.Background(), id).NoEmbed(noEmbed).Prefer(prefer).Execute()
	if err != nil {
...
> ListDisksForAccount200Response ListDisksForAccount(ctx, accountId).Page(page).PerPage(perPage).NoEmbed(noEmbed).Prefer(prefer).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.DisksAPI.ListDisksForAccount(context.Background(), accountId).Page(page).PerPage(perPage).NoEmbed(noEmbed).Prefer(prefer).Execute()
	if err != nil {
